    ...
    @if(class_exists($options->model))
        ...
        @if($options->type == 'belongsTo')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @endphp
                <!--actualizado para mostrar link al clickear -->
                @if(isset($query))
                    <a href="{{ route('voyager.'.$options->table.'.show', $query->id) }}">
                        <p>{{ $query->{$options->label} }}</p></a>
                @else
                    <p>Sin resultados</p>
                @endif
        ...
        @elseif($options->type == 'hasMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    ...
                    $id_values = $model::where($options->column, '=', $relationshipData->id)->pluck('id')->all();
                @endphp
                ...
                @else
                    @if(empty($selected_values))
                        <p>Sin resultados</p>
                    @else
                        <!-- agregar a los links su destino -->
                        <ul>
                            @foreach($selected_values as $i=>$selected_value)

                                <a href="{{ route('voyager.'.$options->table.'.show', $id_values[$i]) }}">
        ...
        @elseif($options->type == 'belongsToMany')
            ...
            @if(isset($view) && ($view == 'browse' || $view == 'read'))
                ...
                @php
                    $table_name=substr_replace($options->table, "", -1);
                    $model = app($options->model);
                    $relationshipData = (isset($data)) ? $data : $dataTypeContent;
                    ...
                    $id_values = isset($relationshipData) ? $relationshipData->belongsToMany($options->model, $options->pivot_table)->pluck($table_name.'_id')->all() : array();
                ...
                @else
                    @if(empty($selected_values))
                        <p>Sin resultados</p>
                    @else
                        <!--actualizado para mostrar link al clickear y mostrar estado de asistencia -->
                        <ul>
                            @foreach($selected_values as $i=> $selected_value)
                                <a href="{{ route('voyager.'.$options->table.'.show', $id_values[$i]) }}">
                                    @if(isset($name))
                                        <br>
                                        <li>{{ $selected_value }}
                                            @if(strtolower($name)=='pasantias' or strtolower($name)=='pasantías')
                                                @if(isset($estadoAsistencias[$id_values[$i]]))
                                                    @if($estadoAsistencias[$id_values[$i]] == 0)
                                                        <span style="font-size: 0.9rem;margin-left: 2px"
                                                              class="label label-danger ">{{ "Ausente" }}</span>
                                                    @else
                                                        <span style="font-size: 0.9rem;margin-left: 2px;"
                                                              class="label label-success ">{{ "Presente" }}</span>
                                                    @endif
